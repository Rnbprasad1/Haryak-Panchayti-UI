...
const MROAdmin = () => {
  ...
  const handleUpdateStatus = (status, adminResponse) => {
    if (selectedData) {
      ...
      const iasIndex = iasDataArray.findIndex((data) => data.token === selectedData.token);
...
const handleDataUpdate = (updatedData) => {
  ...
  const updatedIasDataArray = [...iasDataArray];
  const iasIndex = updatedIasDataArray.findIndex((data) => data.token === updatedData.token);
  if (iasIndex !== -1) {
    updatedIasDataArray[iasIndex] = updatedData;
    setIasDataArray(updatedIasDataArray);
  }
                    ...
                    <Table striped bordered hover>
                      ...
                      <tbody>
                        {sortedComments.map((comment, index) => (
                          <tr key={index}>
                            <td><strong>{comment.role === "User" ? selectedData.name : (comment.role === "IAS" ? selectedData.mandal +'- MRO': comment.role)}</strong></td>
